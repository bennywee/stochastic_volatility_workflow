...
\section{Methodology}
    ...
    \subsection{Evaluation Metrics}
        ...
        \subsubsection{Rank statistics}
            ...
            The shape of the rank statistic distribution gives insight into how a MCMC may be miscalibrated. Specifically, it gives information about the type of bias in the posterior estimates for a given parameter. Figures \ref{fig:overestimate}, \ref{fig:underestimation} and \ref{fig:underdispersed} gives some examples of the posterior estimates which result in bias of the rank statistics.

            Figure \ref{fig:overestimate} shows a distribution of rank statistics with a large left peak. This is consistent with posterior samples overestimating the true parameter. The sum of indicator random variables is small since the majority of posterior draws are larger than the true value. This results in a small rank statistic. Hence, the shape of the histogram is consistent with multiple posterior estimates as part of the SBC procedure being biased towards overestimation of the true parameter. 

            Figure \ref{fig:underestimation} displays the converse behaviour. The large right peak is consistent with a bias towards under estimation of the true parameter. 

            Figure \ref{fig:underdispersed} has large peaks on both ends of the histogram. The sampler is disproportionately over and under estimating the true parameter resulting in biased rank statistics. \citet{talts2018validating} describe this bias as underdispersion of the posterior relative to the prior distribuion (since the posterior draws should follow the same distribution as the prior in this simulation design). The estimated posterior is too narrow relative to the spread of the prior resulting in bias on both ends of the rank statistics distribution. 
            ...
            \begin{figure}[H]
                ...
                \caption{Left: Non uniform rank statistics with peaks on both ends. Right: Overlayed posterior and prior distributions for the same arbitrary parameter (taken from \citet{talts2018validating}). Bias in ranks comes from underdispersed posterior distribution relative to prior.}
