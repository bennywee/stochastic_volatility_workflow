...
path <- "simulation_output/ksc/cp/sir_sbc_cp_ksc_model_cp_dgf_10kmcmc_1000iter_r3"
...
colnames(rank_stats) <- names(rank_bins)
...
additional_parameters <- c("h[1]", "h[250]", "h[500]", "h[750]", "h[900]", "h[1000]")
...
ess_basic_df <- latex_variables(ess_basic_df)
...
ess_bulk_df <- latex_variables(ess_bulk_df)
...
ess_tail_df <- latex_variables(ess_tail_df)
...
rhat_df <- latex_variables(rhat_df)
...
basic_rhat_df <- latex_variables(basic_rhat_df)
