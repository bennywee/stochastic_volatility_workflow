...
If B computation is correct it can capture the known parameers.-->
:::{.nonincremental}
SBC simulates data with known parameters and checks if on average, the interval of an estimated model contains the true parameter

1) Take a draw from the joint prior $\theta^{sim}$
2) Generate a dataset conditional on the prior draw
3) Calculate a rank statistics
4)  Compare the distribution of rank statistics to the discrete uniform distribution which arises when an algorithm is correctly calibrated.
