...
Stochastic volatility models are used in financial econometrics to model the variance or volatility of returns in a financial instrument. The return of a financial asset often exhibits pattterns in the variance (heteroskedasticity) which can be modelled to improve prediction and estimates of risk or tail events. Unlike other volatility models such as ARCH or GARCH, these stochastic volatility models treat the variance as a random variable. Furthermore, these models have a state space representation since the variance is treated as a latent stochastic process. State space models can be difficult to estimate since there is a latent state for every data observation in the time series, which means there are more parameters than data points. These models can be estimated using both frequent tools but this research will focus on Bayesian estimation strategies.  

Kim, Sherpherd and Chib (1998) estimate a discrete time, univariate stochastic volatility model using Bayesian methods which is outlined below. $y_t$ is the mean corrected returns of some asset for equally spaced intervals t. $\beta$ is a constant scaling factor which is also defined as $exp(\mu / 2)$ representing instantaneous volatility. $h_t$ is log volatility, where $h_1$ is a draw from a stationary distribution and the state equation $h_{t+1}$ follows a stationary process governed by the autoregressive parameter $\phi$ such that $|\phi|<1$. This autoregressive parameter represents the persistence or "stickiness" of log volatility and the dispersion parameter $\sigma_{\eta}$ is its variance. $\epsilon_t$ and $\eta_t$ are standard normal white noise shocks. 
...
### Estimation strategy
Kim, Shephard and Chib's sample the posteriors of the stochastic volatility model using a mix of conjugate posterior distributions, Metropolis Hastings and the Kalman Filter and smoother[^1] (de Jong and Shephard, 1995).
...
The goal of this research is to determine which algorithm is better at estimating stochastic volatility models. There are clear differences between Kim Sherphard and Chib's estimation strategy and Hamiltoninan Monte Carlo. My main contribution is to compare these algorithms and determine which of these sampling strategies is better based on whether the algorithm returns calibrated posterior estimates and the efficiency of the MCMC sampler.
...
Since the distribution of the posterior samples follows the prior distribution, the rank statistic for a given parameter and simulation follows a discrete uniform distribution[^2].
...
If the computation is well calibrated and the samples follow a discrete uniform distribution, then the posterior (credible) intervals will have sufficient coverage. That is, for any percentage interval selected (for example 90%) then there is 90\% chance that any $\theta^{sim}$ falls within this interval. In other words, a Bayesian analysis and computation is well calibrated if the 90\% interval contains the true parameter in 90\% interations of the SBC procedure.

## 3.2 Implementation
SBC for Hamiltonian Monte Carlo and the Gaussian approximation are performed with simulated dataset sizes of 1000 simulation iterations, 1000 observations, and post warmup/burnin samples of 999 (which gives 1000 possible rank statistics). This algorithm is summarised below:
...
## 3.3 Metrics
The key metrics and diagnostics to compare the performance of these samplers is the effective sample size, histograms of rank statistics and chi-squared test statistics. 

Effecitve sample size (ESS) measures the efficiency of the MCMC sampler. It calculates the number of (effectively) independent draws from the posterior draws generated by a Markov chain. A poor ESS can arise from high autocorrelation in the Markov chain which leads to highly dependent samples. If it requires long MCMC chains to get to a desired ESS, then this is considered inefficient since it takes more resources to get a representative sample of the target distribution.

Histograms of rank statistics are a visual diagnostic to determine if the distribution of ranks follows a discrete uniform distribution. As described, a calibrated algorithm implies rank statistics which are randomly drawn from a uniform. A weakness of this approach is there are at least as many parameters as there data points. Since there are 1000 observations per dataset, this means there are 1003 parameters to estimate. 
...
The chi squared statistics provide a more succinct summary of the overall performance of the algorithm. 
...
Figure 3 shows results for the Gaussian approximation. The distribution of the rank statistics deviates heavily from the expected discrete uniform distribution, with various peaks at each end of the histogram as well as peaks in the middle. This lack of calibration is expected. The Gaussian approximation only approximates the log chi squared distribution, whereas Hamiltonian Monte Carlo samples directly from the same likelihood as the data generating process.

This result may also be due to the small number of posterior samples drawn from the model. The MCMC sampler may not have converged yet which may explain the deviations from uniformity. A follow up step is to compare the effective sample sizes (ESS) of the two methods and to increase the length of the MCMC chain to ensure the ESS is approximately the same before comparing histograms.  
...
Week 8: Finish off any simulation or modeling not yet complete (listed in above matrix)
