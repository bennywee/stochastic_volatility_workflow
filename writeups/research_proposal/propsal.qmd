...
![Distribution of parameter ranks for HMC algorithm](../../simulation_output/sbc_ncp_ksc_priors_0.999_adapt_delta_premade_datasets_r7_1000_iterations/static_state_hist.png){width=75%}
...
Figure 3 displays the results from KSC's Gaussian approximation. Compared to HMC, the distribution of ranks deviate heavily from the expected discrete uniform distribution. The lack of calibration relative to the HMC algorithm is expected. The Gaussian Mixture Approximiation only approximates the log chi squared distribution. The data generating process is from the specified Stochastic Volatility model. We expect that since the HMC strategy samples directly from the true data generating process that it would have more success at 

![Distribution of parameter ranks for HMC algorithm (5000 iterations)](../../simulation_output/sbc_cp_ksc_model_cp_dgf_r1/static_state_hist.png){width=75%}

# Conclusion and future research considerations

## Plan
The KSC model also also apply a Metropois Hastings correction for the fact that the sampling strategy approximates the error distribution. 

Extensions to the model table:

    + Stan (HMC), KSC (GMA), KSC (GMA + MH correction)
    + CP, NCP

Parameterisation 

## Additional considerations
- Out of sample prediction on real dataset
