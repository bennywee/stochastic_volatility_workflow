...
$$
\begin{aligned}
w(\theta, h) = log\: f(\theta, h | y) - log\:  k(\theta, h_t | y^{\ast}_t) = const + log\: f(y|h) - log\: k(y^{\ast} | h)
\end{aligned}
$$
...
$$
\begin{aligned}
w(\theta, h) = log\: g(\theta, h | y^{\ast}_t) - log\:  k(\theta, h_t | y^{\ast}_t)
\end{aligned}
$$
...
$$
\begin{aligned}
c^l = \frac{exp(w(\theta, h)_l)}{\sum_i exp(w(\theta, h)_i)}
\end{aligned}
$$
...
$$
\begin{aligned}
r &= L \times \sum_{l=1}^{L}1[\theta_{l} < \theta^{sim}]\times c^l   \\
&= L \times \sum_{l=1}^{L}1[\theta_{l} < \theta^{sim}]\times \frac{exp(w(\theta, h)_l)}{\sum_i exp(w(\theta, h)_i)}
\end{aligned}
$$
...
This is way off from zero. However, if you use the log chi squared density  as the "true" density we are trying to approximate, we get the correct plot which they have in their paper. These weights are defined as: $w(\theta, h) = log\: g(\theta, h_t | y^{\ast}_t) - log\:  k(\theta, h_t | y^{\ast}_t)$
