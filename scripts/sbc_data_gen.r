...
if(parameterisation == "cp"){
    dgf <-  gen_cp_sv_dataset
} else if (parameterisation == "ncp") {
   dgf <-  gen_ncp_sv_dataset
} else {
    stop("Incorrect parameterisation")
}

set.seed(seed)
seeds <- sample.int(1000000)[1:n_datasets] 
lapply(1:n_datasets, dgf, seed_list = seeds, T = n_time_points)
